...
public final class PlatformTool {
    ...
    public static final boolean isX11() {
        ...
        return !PlatformTool.isWindows() && !PlatformTool.isMac();
    }
    ...
    static String getCanonicalArchitecture(String arch, int platform) {
        ...
        if("arm".equals(arch) && platform == PlatformTool.LINUX && isSoftFloat()) {
            arch = "armel";
        }
    ...
    static boolean isSoftFloat() {
        try {
            ...
            if (self.exists()) {
                ...
                Class<?> elfanalizer = Class.forName("com.sun.jna.ELFAnalyser");
                Method analyze = elfanalizer.getMethod("analyse", String.class);
                Method isArmHardFloat = elfanalizer.getMethod("isArmHardFloat");

                analyze.setAccessible(true);
                isArmHardFloat.setAccessible(true);

                Object o = analyze.invoke(null, self.getCanonicalPath());
                return !(boolean) isArmHardFloat.invoke(o);
            }
        } catch (IOException | ReflectiveOperationException ex) {
            // asume hardfloat
            Logger.getLogger(PlatformTool.class.getName()).log(Level.INFO, "Failed to read '/proc/self/exe' or the target binary.", ex);
        } catch (SecurityException ex) {
            // asume hardfloat
            Logger.getLogger(PlatformTool.class.getName()).log(Level.INFO, "SecurityException while analysing '/proc/self/exe' or the target binary.", ex);
        }
        return false;
    }
    ...
    */
    static String getNativeLibraryResourcePrefix(int osType, String arch, String name) {
        ...
        switch(osType) {
            case PlatformTool.ANDROID:
                if (arch.startsWith("arm")) {
                    arch = "arm";
                }
                osPrefix = "android-" + arch;
                break;
            case PlatformTool.WINDOWS:
                osPrefix = "win32-" + arch;
                break;
            case PlatformTool.WINDOWSCE:
                osPrefix = "w32ce-" + arch;
                break;
            case PlatformTool.MAC:
                osPrefix = "darwin-" + arch;
                break;
            case PlatformTool.LINUX:
                osPrefix = "linux-" + arch;
                break;
            case PlatformTool.SOLARIS:
                osPrefix = "sunos-" + arch;
                break;
            case PlatformTool.FREEBSD:
                osPrefix = "freebsd-" + arch;
                break;
            case PlatformTool.OPENBSD:
                osPrefix = "openbsd-" + arch;
                break;
            case PlatformTool.NETBSD:
                osPrefix = "netbsd-" + arch;
                break;
            case PlatformTool.KFREEBSD:
