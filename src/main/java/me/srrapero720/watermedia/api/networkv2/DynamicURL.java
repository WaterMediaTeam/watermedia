...
public final class DynamicURL {
    ...
    DynamicURL(String source, URL url, File file) {
        this.source = source;
        this.url = url;
        this.file = file;
    }

    DynamicURL(String urlOrPath) {
    ...
    DynamicURL(final String urlOrPath, final boolean isVideo, final boolean isStream) {
        this(urlOrPath);
        this.video = isVideo;
        this.stream = isStream;
    }

    DynamicURL(final File file) {
        this.source = file.getAbsolutePath();
        this.file = file;
        this.url = NetworkAPI.parseUrl(file.toURI());
    }

    DynamicURL(final Path path) {
        this(path.toFile());
    }
