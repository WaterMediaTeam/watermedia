        ...
        if (matcher.find()) {
            try {
                ...
                if (videoDetails.isLive()) {
                    ...
                    if (bestAll != null) return new DynamicURL(bestAll.url(), true, false);

                    // TODO: add quality support
                    // WITHOUT AUDIO
                    VideoFormat bestWithoutAudio = videoInfo.bestVideoFormat();
                    if (bestWithoutAudio != null) {
                        return new DynamicURL(bestWithoutAudio.url(), true, false);
                    }

                    // TODO: add quality support
                    // WITHOUT VIDEO
                    AudioFormat bestWithoutVideo = videoInfo.bestAudioFormat();
                    if (bestWithoutVideo != null) return new DynamicURL(bestWithoutAudio.url()), true, false).setAudioTrack(new URL(bestWithoutVideo.url()));
