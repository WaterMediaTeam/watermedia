...
package me.lib720.caprica.media.events;
...
import me.lib720.caprica.binding.internal.media_attached_thumbnails_found;
...
import me.lib720.caprica.media.Picture;
...
final class MediaAttachedThumbnailsFoundEvent extends MediaEvent {
    ...
    public void notify(MediaEventListener listener) {
        int count = LibVlc.libvlc_picture_list_count(thumbnails).intValue();
        List<Picture> pictures = new ArrayList<>(count);
        for (int i = 0; i < count; i++) {
            libvlc_picture_t picture = LibVlc.libvlc_picture_list_at(thumbnails, new size_t(i));
