...
public final class ThumbnailApi extends BaseApi {
    ...
    public ThumbnailRequest requestByTime(long time, ThumbnailerSeekSpeed speed, int width, int height, boolean crop, PictureType pictureType, long timeout) {
        libvlc_media_thumbnail_request_t request = LibVlc.libvlc_media_thumbnail_request_by_time(libvlcInstance, mediaInstance, time, speed.intValue(), width, height, crop ? 1 : 0, pictureType.intValue(), timeout);
        if (request != null) {
    ...
    public ThumbnailRequest requestByPosition(double position, ThumbnailerSeekSpeed speed, int width, int height, boolean crop, PictureType pictureType, long timeout) {
        libvlc_media_thumbnail_request_t request = LibVlc.libvlc_media_thumbnail_request_by_pos(libvlcInstance, mediaInstance, position, speed.intValue(), width, height, crop ? 1 : 0, pictureType.intValue(), timeout);
        if (request != null) {
            return new ThumbnailRequest(request);
        } else {
            return null;
