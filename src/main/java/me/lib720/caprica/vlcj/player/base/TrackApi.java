...
import static me.lib720.caprica.vlcj.media.TrackType.trackType;
...
public final class TrackApi extends BaseApi {
    ...
    public VideoTrackList videoTracks(boolean selected) {
        libvlc_media_tracklist_t trackList = LibVlc.libvlc_media_player_get_tracklist(mediaPlayerInstance, TrackType.VIDEO.intValue(), selected ? 1 : 0);
        if (trackList != null) {
    ...
    public AudioTrackList audioTracks(boolean selected) {
        libvlc_media_tracklist_t trackList = LibVlc.libvlc_media_player_get_tracklist(mediaPlayerInstance, TrackType.AUDIO.intValue(), selected ? 1 : 0);
        if (trackList != null) {
    ...
    public TextTrackList textTracks(boolean selected) {
        libvlc_media_tracklist_t trackList = LibVlc.libvlc_media_player_get_tracklist(mediaPlayerInstance, TrackType.TEXT.intValue(), selected ? 1 : 0);
        if (trackList != null) {
    ...
    public VideoTrack selectedVideoTrack() {
        libvlc_media_track_t track = LibVlc.libvlc_media_player_get_selected_track(mediaPlayerInstance, TrackType.VIDEO.intValue());
        if (track != null) {
    ...
    public AudioTrack selectedAudioTrack() {
        libvlc_media_track_t track = LibVlc.libvlc_media_player_get_selected_track(mediaPlayerInstance, TrackType.AUDIO.intValue());
        if (track != null) {
    ...
    public TextTrack selectedTextTrack() {
        libvlc_media_track_t track = LibVlc.libvlc_media_player_get_selected_track(mediaPlayerInstance, TrackType.TEXT.intValue());
        if (track != null) {
    ...
    public Track track(String trackId) {
        libvlc_media_track_t track = LibVlc.libvlc_media_player_get_track_from_id(mediaPlayerInstance, trackId);
        if (track != null) {
    ...
    public void selectTrack(Track track) {
        LibVlc.libvlc_media_player_select_track(mediaPlayerInstance, track.instance());
    }
    ...
    public void select(TrackType type, String... trackIds) {
        if (trackIds != null && trackIds.length > 0) {
            LibVlc.libvlc_media_player_select_tracks_by_ids(mediaPlayerInstance, type.intValue(), String.join(",", trackIds));
        }
    }
    ...
    public void deselect(TrackType... types) {
        if (types != null && types.length > 0) {
            for (TrackType type : types) {
                LibVlc.libvlc_media_player_unselect_track_type(mediaPlayerInstance, type.intValue());
            }
        }
    }
    ...
    private void select(TrackType type, Track[] tracks) {
        ...
        if (!list.isEmpty()) {
            LibVlc.libvlc_media_player_select_tracks(mediaPlayerInstance, type.intValue(), getPointerArray(tracks), new size_t(list.size()));
        }
    }
