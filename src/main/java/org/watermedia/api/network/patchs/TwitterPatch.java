...
public class TwitterPatch extends AbstractPatch {
    ...
    @Override
    public void patch(MRL source) throws PatchException {
        try {
            ...
            try (final InputStream in = conn.getInputStream()) {
                ...
                for (MediaDetail details: tweet.mediaDetails) {
                    ...
                    switch (details.type) {
                        case "photo" -> {
                            mediaSource.setType(MediaPlayer.Type.IMAGE);
                            mediaSource.setUri(new URI(details.mediaUrlHttps));
                        }
                        case "video" -> {
                            mediaSource.setType(MediaPlayer.Type.VIDEO);
                            for (VideoVariant videoVariant: details.videoInfo.variants) {
                                ...
                                if (videoVariant.url.contains(".mp4") && matcher.find()) {
                                    ...
                                    mediaSource.putQualityIfAbsent(MediaPlayer.Quality.calculate(Integer.parseInt(width)), new URI(videoVariant.url));
                                    mediaSource.setFallbackUri(new URI(details.mediaUrlHttps));
                                    mediaSource.setFallbackType(MediaPlayer.Type.IMAGE);
                                }
                            }
                        }
