...
public class ImagePlayer extends MediaPlayer {
    private static final List<ImagePlayer> ACTIVE_MEDIA = new ArrayList<>();
    ...
    public ImagePlayer(BufferedImage[] images, long[] delays) {
        this(
    ...
    public ImagePlayer(ByteBuffer buffer, int width, int height) {
        this(new ByteBuffer[] { buffer }, new int[] { width }, new int[] { height }, new long[1]);
    }

    public ImagePlayer(ByteBuffer[] buffers, int[] width, int[] height, long[] delays) {
        this.widths = width;
        this.heights = height;
